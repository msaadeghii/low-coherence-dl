function [D,X,err,gerr,ratio,mucd] = RINC_DL(params,varargin)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ((IMPORTANT)) Copyright notice: The main body of the code has been written by Ron Rubinstein (ronrubin@cs)
% as an implementation of the ksvd algorithm, and it is part of the ksvd toolbox.
% We have written our own algorithms by modifying appropriate parts of
% the original ksvd code.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Regularized Incoherent Dictionary Learning (RINC-DL) algorithm.
%  [D,GAMMA] = RINC-DL(PARAMS) runs the RINC-DL dictionary training algorithm, which is a low-coherence
%  DL algorithm proposed in [1].
%
%  on the specified set of signals, returning the trained dictionary D and the
%  signal representation matrix GAMMA. It targets the following
%  minimization problem:
%
%  min_{D,Gamma} 1/2.|X-D*GAMMA|_F^2+lam.|D'*D-I|_inf
%
% in which, lam>=0 is a regularization parameter.
%
%  RINC-DL has two modes of operation: sparsity-based and error-based. For
...
%
%  [D,GAMMA,ERR] = RINC-DL(PARAMS) also returns the target function values
...
%
%  [D,GAMMA,ERR,GERR] = RINC-DL(PARAMS) computes the target function values on
...
%
%  [...] = RINC-DL(...,VERBOSE) where VERBOSE is a character string, specifies
...
%  example, RINC-DL(PARAMS,'tr') prints the iteration number, number of
...
%
%  [...] = RINC-DL(...,MSGDELTA) specifies additional messages to be printed
...
%  causes RINC-DL to run in silent mode, ignoring the value of MSGDELTA.
%
%  [1] Mostafa Sadeghi and Massoud Babaie-Zadeh,
...
%      Specifies the number of RINC-DL iterations to perform. If not
...
lam=params.lam; % the regularization parameter
itermu=params.itermu; % maximum number of outer-loop iterations
[~,K]=size(D);
Y=data;
I=eye(K);
Z1=zeros(K);
...
Z=D'*D;
G1=Z-I;
c=params.c;     % decreasing factor of the alpha sequence
J=params.J;     % number of inner-loop iterations
tol=params.tol; % tolerance for terminating the outer-loop
taub = params.taub;
cb = params.cb;
maxiterb = params.maxiterb;
...
for iter = 1:iternum
    ...
    X = sparsecode(Y,D,XtX,G,thresh);
    if (comperr)
        err(iter) = compute_err(D,X,Y);
    end
    if cmpe
        err(iter) =20*log10(ndata/norm(Y-D*X,'fro'));
    end
    ...
    while errGD > tol  &&  (ii <= itermu)

        muD=2/(svdsx+1/(alpha*params.muD));
        ...
        for jj=1:J
            ...
            if  lam<1e-5
                ...
                for i=1:K
                    Z1(:,i)=ProjectOntoL1Ball(Z0(:,i),lam*alpha);
                end
            end
            ...
            f = @(D) 0.5*norm(Y-D*X,'fro')^2+0.5/alpha*norm(D'*D-Z,'fro')^2;
            p = -(D*XXT-YXT+(2/alpha)*D*(D'*D-Z));
            [D, muD] = mybacktracking(D, f, 1e0*muD, taub, cb, p, maxiterb);

        end

        errGD=norm(Z-D'*D,'fro');
        alpha=alpha*c;
        ii=ii+1;
    end
    G1=Z0;
    ...
    %%%%%  print info  %%%%%
    %
    info = sprintf('Iteration %d / %d complete. MC: %0.4f    Err: %0.4f   Iter:  %0.2f', iter, iternum, mucd(iter), err(iter)...
        ,ii);
    if (printerr)
...
% function [Dnew, muD] = mybacktracking(D, f, muD0, tau, c, p, maxiter)
%
% iter = 1;
% muD = muD0;
% t = -c*norm(p,'fro')^2;
...
% Dnew = normc(D+muD*p);
%
% while iter <= maxiter && f(D)-f(Dnew) < muD*t
%        sprintf([num2str(iter),'----', num2str(muD)])
%        muD = muD*tau;
%        iter = iter+1;
%        Dnew = normc(D+muD*p);
% end
