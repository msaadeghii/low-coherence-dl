function [D,Gamma,err,gerr,ratio,muCD] = BSC_DL(params,varargin)
...
% Bounded Self-Coherence Dictionary Learning (BSC-DL) algorithm.
%  [D,GAMMA] = BSC-DL(PARAMS) runs the BSC-DL dictionary training algorithm, which is a low-coherence
%  DL algorithm proposed in
%   
%  [*] C. D. Sigg, T. Dikk, and J. M. Buhmann, “Learning dictionaries with
%      bounded self-coherence,” IEEE Signal Proc. Letters, vol. 19, no. 12,
%      pp. 861–864, 2012.
%
...
%   
%  min_{D,Gamma} 1/2.|X-D*GAMMA|_F^2+lam.|D'*D-I|_F^2
%
% in which, lam>=0 is a regularization parameter.
%
%  BSC-DL has two modes of operation: sparsity-based and error-based. For
...
%
%  [D,GAMMA,ERR] = BSC-DL(PARAMS) also returns the target function values
...
%
%  [D,GAMMA,ERR,GERR] = BSC-DL(PARAMS) computes the target function values on
...
%
%  [...] = BSC-DL(...,VERBOSE) where VERBOSE is a character string, specifies
...
%  example, BSC-DL(PARAMS,'tr') prints the iteration number, number of
...
%
%  [...] = BSC-DL(...,MSGDELTA) specifies additional messages to be printed
...
%  causes BSC-DL to run in silent mode, ignoring the value of MSGDELTA.
%
%
...
%      Specifies the number of BSC-DL iterations to perform. If not
...
for iter = 1:iternum
    ...
    Gamma = sparsecode(Y,D,XtX,G,thresh);
    if (comperr)
        err(iter) = compute_err(D,Gamma,Y);
    end
    if cmpe
        err(iter) =20*log10(ndata/norm(Y-D*Gamma,'fro'));
    end
    
    %%%%%  dictionary update  %%%%%
    D = fminlbfgs(@myfgrad,D,options);
    D = normc(D);
    muCD(iter)=mucoh(D);
    ...
    info = sprintf('Iteration %d / %d complete. MC: %0.4f    Err: %0.4f ', iter, iternum, mucoh(D), err(iter));
    if (printerr)
...
function [f, g]=myfgrad(D)
        f=norm(data-D*Gamma,'fro')^2+lam*0.5*norm((D'*D)-eye(dictsize),'fro')^2;
        if ( nargout > 1 )
      	g=2*(D*Gamma-data)*Gamma'+2*lam*D*((D'*D)-eye(dictsize));
  	    end
end
end
