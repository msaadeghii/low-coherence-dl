function [D,Gamma,err,gerr,ratio,muCD] = IPR_DL(params,varargin)
...
% Iterative Projections and Rotations Dictionary Learning (IPR-DL) algorithm.
%  [D,GAMMA] = IPR-DL(PARAMS) runs the IPR-DL dictionary training algorithm, which is a low-coherence
%  DL algorithm proposed in
%   
%  [*] D. Barchiesi and M. D. Plumbley, �Learning incoherent dictionaries for
%      sparse approximation using iterative projections and rotations,� IEEE
%      Trans. on Signal Proc., vol. 61, no. 8, pp. 2055�2065, 2013.
%
...
%
%  IPR-DL has two modes of operation: sparsity-based and error-based. For
...
%
%  [D,GAMMA,ERR] = IPR-DL(PARAMS) also returns the target function values
...
%
%  [D,GAMMA,ERR,GERR] = IPR-DL(PARAMS) computes the target function values on
...
%
%  [...] = IPR-DL(...,VERBOSE) where VERBOSE is a character string, specifies
...
%  example, IPR-DL(PARAMS,'tr') prints the iteration number, number of
...
%
%  [...] = IPR-DL(...,MSGDELTA) specifies additional messages to be printed
...
%  causes IPR-DL to run in silent mode, ignoring the value of MSGDELTA.
%
%
...
%      Specifies the number of IPR-DL iterations to perform. If not
...
muCD=zeros(1,iternum); % mutual coherence vector
...
mu0=params.mu0;  % desired level of mutual coherence
itermu=params.itermu;
[n,m]=size(D);
...
for iter = 1:iternum
    ...
    s1=(data*Gamma');
    s2=1e0*eye(dictsize); %(Gamma*Gamma')+
    D=s1/s2;
    D = normc(D);
    muCD(iter)=mucoh(D);
    Dold = D;
    % Perform IPR
    mucohD=zeros(1,itermu);
    ii=1;
    
    while (ii <= itermu) && (mucoh(D) > mu0+1e-3 )
        
        G = D'*D;
        
        % project onto the structural constraint set
        H = zeros(size(G));				%initialise matrix
        ind1 = find(abs(G(:))<=mu0);		%find elements smaller than mu
        ind2 = find(abs(G(:))>mu0);		%find elements bigger than mu
        H(ind1) = G(ind1);				%copy elements belonging to ind1
        H(ind2) = mu0*sign(G(ind2));		%threshold elements belonging to ind2
        H(1:m+1:end) = 1;				%set diagonal to one
        
        % project into spectral constraint set
        [V, S ] = eig(H);
        %G = alpha*(V(:,1:n)*V(:,1:n)');
        S1=S(end-n+1:end,end-n+1:end);
        S1(S1<1e-5)=0;
...
%             error('Null!!!');
    ...
    info = sprintf('Iteration %d / %d complete. MC: %0.4f . Err: %0.6f', iter, iternum, mucoh(D),err(iter));
    if (printerr)
