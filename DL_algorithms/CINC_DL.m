function [D,X,err,gerr,ratio,mucd] = CINC_DL(params,varargin)
...
% Constrained Incoherent Dictionary Learning (CINC-DL) algorithm.
%  [D,GAMMA] = CINC-DL(PARAMS) runs the CINC-DL dictionary training algorithm, which is a low-coherence
...
%
%  min_{D,Gamma} 1/2.|X-D*GAMMA|_F^2   s.t.   mu(D)<=mu0
%
% in which, 0<mu0<=1 is a desired level of mutual coherence.
%
%  CINC-DL has two modes of operation: sparsity-based and error-based. For
...
%
%  [D,GAMMA,ERR] = CINC-DL(PARAMS) also returns the target function values
...
%
%  [D,GAMMA,ERR,GERR] = CINC-DL(PARAMS) computes the target function values on
...
%
%  [...] = CINC-DL(...,VERBOSE) where VERBOSE is a character string, specifies
...
%  example, CINC-DL(PARAMS,'tr') prints the iteration number, number of
...
%
%  [...] = CINC-DL(...,MSGDELTA) specifies additional messages to be printed
...
%  causes CINC-DL to run in silent mode, ignoring the value of MSGDELTA.
%
...
%      Specifies the number of CINC-DL iterations to perform. If not
...
muC=params.mu0; % desired level of mutual coherence
...
for iter = 1:iternum
    ...
    %%%%%  print info  %%%%%
    %
    info = sprintf('Iteration %d / %d complete. MC: %0.4f    Err: %0.4f ', iter, iternum, mucd(iter), err(iter));
    if (printerr)
